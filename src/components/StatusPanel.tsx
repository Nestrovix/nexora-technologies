    ...
    <div className={className}>
      <div className="panel-dark min-w-0">
        <div className="panel-dark__bar">
          ...
          <span className="ml-auto shrink-0 rounded-sm border border-term-line px-2 py-1 font-mono text-[10px] uppercase leading-none tracking-[0.12em] text-term-warn">
            Sample data
          </span>
        </div>
          ...
          {serviceMetrics.map((m) => {
            ...
            return (
              <div key={m.key} className="bg-term-bg px-4 py-5">
                <dt className="flex items-center gap-2 font-mono text-[11px] uppercase tracking-[0.12em] text-term-dim">
                  <span className={`dot ${tone.dot}`} aria-hidden="true" />
                  {m.key}
                </dt>
                <dd className="mt-3">
                  <span className="block font-mono text-2xl font-medium tabnum text-term-text">{m.value}</span>
                  <span className={`mt-2 block font-mono text-[11px] ${tone.text}`}>{m.target}</span>
                  <span className="mt-2 block font-mono text-[11px] leading-relaxed text-term-dim">{m.note}</span>
                </dd>
              </div>
            );
          })}
        ...
        <p className="border-t border-term-line px-4 py-3 font-mono text-[11px] leading-relaxed text-term-dim">
